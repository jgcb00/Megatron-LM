...
@dataclass
class SambaStackSubmodules:
    """
    A class for the module specs for the SambaStack.
    """
...
class SambaStack(MegatronModule):
    """
    Constructor for the SambaStack class.
    ...
    def __init__(
        ...
        for _ in range(num_layers_per_pipeline_rank):
            ...
            layer = build_module(submodules.mlp_layer, config=self.config, layer_number=len(self.layers) + 1)
            self.layers.append(layer)
            # Transformer layers apply their own pp_layer_offset
            layer = build_module(submodules.attention_layer, config=self.config, layer_number=len(self.layers) + 1)
            self.layers.append(layer)
            # Transformer layers apply their own pp_layer_offset
            layer = build_module(submodules.mlp_layer, config=self.config, layer_number=len(self.layers) + 1)
            self.layers.append(layer)
